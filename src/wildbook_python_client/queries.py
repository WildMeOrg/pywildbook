...
def filter_by_species(genus: str, specific_epithet: Optional[str] = None) -> Dict[str, Any]:
    """Create a query to filter by species.

    Args:
        genus: Genus name
        specific_epithet: Specific epithet (optional)
    ...
    if specific_epithet:
        return {
            'bool': {
                'must': [
                    {'term': {'genus': genus}},
                    {'term': {'specificEpithet': specific_epithet}}
                ]
            }
        }
    else:
        return {'term': {'genus': genus}}
